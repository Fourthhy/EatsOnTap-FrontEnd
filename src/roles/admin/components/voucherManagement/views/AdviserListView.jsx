...
export const AdviserListView = ({ switcher }) => {
    ...
    const handleAddAdviser = (newAdviser) => {
        const entry = { id: `adv-${Date.now()}`, ...newAdviser, years: parseInt(newAdviser.years) || 0 };
        setAdvisersData(prev => [entry, ...prev]);
    };
    const handleRowClick = (adviser) => {
        if (isEditing) return; 
        if (selectedItem?.id === adviser.id) { setSelectedItem(null); setIsActionBarVisible(false); }
        else { setSelectedItem(adviser); setIsActionBarVisible(true); }
    };
    const handleEditClick = () => { setEditFormData({ ...selectedItem }); setIsEditing(true); };
    const handleSaveEdit = () => {
        setAdvisersData(prev => prev.map(item => item.id === selectedItem.id ? editFormData : item));
        setSelectedItem(editFormData); setIsEditing(false);
    };
    const handleCancelEdit = () => { setIsEditing(false); setEditFormData({}); };
    const handleDepartmentChange = (newDept) => {
        const availableSections = DEPARTMENT_SECTIONS[newDept] || [];
        setEditFormData({ ...editFormData, department: newDept, assignment: availableSections.length > 0 ? availableSections[0] : '' });
    };
            ...
            return (
                <tr key={adviser.id} style={{ backgroundColor: '#eff6ff' }}>
                    ...
                    <td style={{...cellStyle, paddingRight: "10px"}}>
                        <input className="w-full border rounded text-xs focus:ring-1 focus:ring-[#4268BD] outline-none transition-all" style={{ padding: '8px 0px' }} value={editFormData.name} onChange={e => setEditFormData({...editFormData, name: e.target.value})} autoFocus />
                    </td>
                    <td style={cellStyle}><CustomEditDropdown value={editFormData.department} options={DEPARTMENTS} onChange={handleDepartmentChange} /></td>
                    <td style={cellStyle}><CustomEditDropdown value={editFormData.assignment} options={currentDeptSections} onChange={v => setEditFormData({...editFormData, assignment: v})} /></td>
                </tr>
            );
        }
        const isSelected = selectedItem?.id === adviser.id;
        return (
            <tr key={adviser.id} className="hover:bg-gray-50 transition-colors cursor-pointer" onClick={() => handleRowClick(adviser)} style={{ backgroundColor: isSelected ? '#eff6ff' : 'transparent' }}>
    ...
    const actionBar = (
        <AnimatePresence>
            {shouldShowActions && (
                <SelectionActionBar 
                    key="bar" variant="student" selectedItem={selectedItem}
                    onClearSelection={() => { setSelectedItem(null); setIsEditing(false); }}
                    isEditing={isEditing} onEditStudent={handleEditClick} onSaveStudent={handleSaveEdit} onCancelEdit={handleCancelEdit}
                    activeDropdown={activeActionDropdown} onToggleDropdown={t => setActiveActionDropdown(prev => prev === t ? null : t)} onArchiveOption={(opt) => console.log("Adviser Action:", opt)}
                    selectedCount={1}
                />
            )}
        </AnimatePresence>
    ...
    return (
        <>
            ...
            <GenericTable
                ...
                onPrimaryAction={() => setIsAddModalOpen(true)} primaryActionLabel="Add Teacher" primaryActionIcon={<Plus size={16} />}
