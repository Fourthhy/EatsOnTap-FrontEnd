...
export const UpdateRecordsModal = ({ isOpen, onClose }) => {
    ...
    return (
        <div className={`fixed inset-0 z-[9999] flex items-center justify-center transition-opacity ${isClosing ? 'opacity-0' : 'opacity-100'} backdrop-blur-sm bg-black/50`} onClick={handleClose}>
            <div className={`bg-white rounded-xl shadow-2xl w-full max-w-2xl flex flex-col transform ${animationClass}`} onClick={(e) => e.stopPropagation()} style={{ maxHeight: '90vh', fontFamily: "inherit" }}>
                ...
                <div className="flex justify-between items-center border-b border-gray-100" style={{ padding: '24px' }}>
                    ...
                    <button onClick={handleClose} className="text-gray-400 hover:text-gray-600 rounded-full hover:bg-gray-50 transition-colors" style={{ padding: '8px' }}>
                        <X size={20} />
                    </button>
                </div>
                            ...
                            <motion.div key="step2" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="flex flex-col" style={{ gap: '12px' }}>
                                ...
                                {completedSteps.length === currentSteps.length && (
                                    <div className="bg-green-100 border border-green-300 rounded-xl text-center animate-in fade-in zoom-in duration-500" style={{ marginTop: '16px', padding: '24px' }}>
                                        ...
                                        <button onClick={handleClose} className="bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium" style={{ marginTop: '16px', padding: '8px 24px' }}>Close Window</button>
                                    </div>
                                )}
                            </motion.div>
                        )}
